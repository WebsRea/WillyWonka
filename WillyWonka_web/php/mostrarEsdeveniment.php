...
// }else{
      ...
      if ($row = mysqli_fetch_array($results)){ 
         ...
         echo "<tr class='datos_tabla padmin'><td>Títol</td><td>Descripció</td><td>Inici</td><td>Fi</td><td>Acció</td></tr> \n"; 
         do { 
            echo "<tr><td>".$row["esd_titol"]."</td><td>".$row["esd_text"]."</td><td>".$row["esd_data_ini"]."</td><td>".$row["esd_data_fin"]."</td><td><a href='procs/eliminarEsdeveniment.proc.php?esd_id=".$row['esd_id']."''>eliminar</a></td></tr> \n"; 
         } while ($row = mysqli_fetch_array($results)); 
         echo "</table> \n"; 
      } else { 
      echo "<img src='../img/icon/001-sad.png'> No hi ha cap registre!"; 
